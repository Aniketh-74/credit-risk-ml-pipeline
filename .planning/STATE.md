...
**Current focus:** Phase 4 — Proving the Feedback Loop

## Current Position

Phase: 4 of 7 (Proving the Feedback Loop) — IN PROGRESS
Plan: 1 of 3 — plan 04-01 complete
Status: Phase 4 Plan 1 delivered — denial loop and score gaming simulators
Last activity: 2026-04-05 — 04-01 executed: DenialLoopSimulator, ScoreGamingSimulator, 4 smoke tests (all green)

Progress: [████░░░░░░] 33% (Phase 4, Plan 1 of 3)
...
- [Phase 03-01]: MonthlyIncome and NumberOfDependents declared Optional in LoanApplicationRequest — 19.8%/2.6% NaN in training; None→NaN→median via imputer, avoids train-serve skew vs requiring callers to pass 0.0
- [Phase 03-01]: Module-scope Session = sessionmaker() in db.py — request-scoped DI sessions close before BackgroundTask runs causing DetachedInstanceError; factory pattern creates fresh sessions per write
- [Phase 03-01]: DB predictions.decision CHECK constraint uses 'approved'/'denied' (past tense); API response contract uses 'approve'/'deny' (present tense) — must map explicitly in /score handler
- [Phase 04-01]: Outcome.predicted_at is NOT NULL with no server default — simulators must always set predicted_at on outcome rows explicitly (omission causes IntegrityError on both SQLite and PostgreSQL)
- [Phase 04-01]: UUID generated in Python (str(uuid.uuid4())) in all simulator rows — gen_random_uuid() is PostgreSQL-only and breaks SQLite test engine; explicit UUID enables cross-engine testing
- [Phase 04-01]: ScoreGamingSimulator score_fn is injectable — test-time mock controls approval rates; Phase 5 will inject champion model callable from MLflow
...
Stopped at: 04-01 complete — DenialLoopSimulator, ScoreGamingSimulator, 4 smoke tests committed
Resume file: .planning/phases/04-proving-the-feedback-loop/04-02-PLAN.md
